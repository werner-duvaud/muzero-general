...
class MuZeroConfig:
    def __init__(self):
        ...
        numJoints=2
        maxSteps = 50
        self.action_space = numpy.ones(numJoints) #models.py just does len of this variable to determine the action space
        # #range of action space doesn't matter because gaussian network will create actions without regard to limits  # Fixed list of all possible actions. You should only edit the length
        ...
        self.support_size = 13  # Value and reward are scaled (with almost sqrt) and encoded on a vector with a range of -support_size to support_size
...
class Game(AbstractGame):
    ...
    def legal_actions(self):
        #This is called in a few places in self_play but it does not affect the training
        #if code is being re-published its probably best to remove this function to avoid confusion
        """
